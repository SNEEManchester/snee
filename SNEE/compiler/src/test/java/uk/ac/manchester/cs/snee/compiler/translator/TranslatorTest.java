...
//			System.out.println(op.getOperatorName());
	...
	/* There is no source for forestLRF and that is a valid exception.
	 * However, intensional extents SHOULD NOT be matched to a source.
	 * Look in the SourceManager class, in the getSource() method, to
	 * see the comment for this reason */
	...
	OptimizationException, TypeMappingException 
	{
		LAF laf = testQuery( "SELECT RSTREAM F.integerColumn, RF.temperature " +
				"FROM PushStream[FROM NOW-10 seconds TO NOW SLIDE 10 seconds] F, forestLRF RF " +
				"WHERE F.integerColumn=RF.moisture;");
		LogicalOperator rootOperator = laf.getRootOperator();
		List<Attribute> attributes = 
			rootOperator.getAttributes();
		assertEquals(1, attributes.size());
