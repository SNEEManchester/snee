...
class D3OutlierDetection extends AbstractOutlierDetection {
	 ...
	 * which are used to construct the D3 Outlier Detection algorithm, 
	 * */
...
//		range = 5;
	...
	/**
	 * This is the public constructor for the D3 Outlier Detection algorithm. The input
	 * is an array of Strings, containing the parameters that the D3 algorithm requires to
	 * operate
	 * 
	 * @param modelName The name of the model that the user specified when writing the query
	 * 
	 * @param input Contains the parameters that D3 requires. input[0] Always contains the
	 * range that D3 has. input[1] is optional, and if it exists, it is a probability value
	 * that we would like to check against. Provided that the probability is given, the 
	 * handling should be different
	 * 
	 * @throws DATException
	 * 
	 * */
	public D3OutlierDetection(String modelName, String[] input, String outputParam, 
		...
		range = Integer.parseInt(input[0]);
	}

	/** Validates the parameters that have been passed to this object to create a new D3 outlier
	 * detection algorithm.
	 * 
	...
	protected void validateParameters() throws DATException, ParserException {
		...
		input_Xi = new String[selArgs.length];
		for ( int i = 0; i < selArgs.length; i++ )
			input_Xi[i] = selArgs[i].replace(tableIter, "");
	}
	...
	private String getQ1(){
		...
		for ( int i = 0; i < input_Xi.length; i++ ){
			sb.append("SUM(q0.").append(input_Xi[i]).append(") as sum").append(i + 1).
			append(",AVG(q0.").append(input_Xi[i]).append(") as avg").append(i + 1).append(',');
		}
		sb.append("COUNT(*) as cnt FROM (").append( datSource ).append(") q0");
		return sb.toString();
	}
	...
	private String getQ6(String joinTable, String joinTableItr, int range){
		...
		sb.setCharAt(sb.length() - 1, ')');
		sb.append("*q5.q as ").append(derivedAttributeName).append(
				" FROM (").append( getQ5(joinTable, joinTableItr, range) ).append(") q5");
