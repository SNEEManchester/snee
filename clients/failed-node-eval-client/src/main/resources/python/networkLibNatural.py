		...
		if self.edges == None:	
			...
			for i in range(0,numNodes):
				for j in range(0,numNodes):
					...
					if powerSetting < 255 and self.edgeNeeded(i,j):
						self.edges[i][j] = powerSetting
						self.edges[j][i] = powerSetting
					if powerSetting > 255 and powerSetting < 300:
						self.edges[i][j] = 230
						self.edges[j][i] = 230
			...
			for i in range(0,numNodes):
				if len(self.edges[i].keys()) == 0:
					for j in range(0, 5):
						power = random.randrange(0,230,1)
						k = random.randrange(0, numNodes -1, 1)
						if i == k:
							k = k -1
		
						self.edges[i][k] = power
						self.edges[k][i] = power
					
		self.checkForDisconnectedSections()
			# print self.edges
	def pathFind(self, node, j):
		connectedNodes = []
		foundNodes = [node]
		index = 0
		while index < (len(foundNodes) -1):
			for j in range(0, (len(self.edges[foundNodes[index]].keys())) -1):
				...
				if k == 0:
					...
					if k not in foundNodes:
						foundNodes.append(k)
			index = index + 1
		return foundNodes
	
	def checkForDisconnectedSections(self):
		numNodes = len(self.nodes.keys())
		for i in range(1, numNodes):
			disconnectedNodes = self.pathFind(i, 0)
			if len(disconnectedNodes) != 0:
				connection = random.sample(self.nodes, 1)[0]
				while(connection in disconnectedNodes):
					connection = random.sample(self.nodes, 1)[0]
				cost = random.randrange(0,254,1)
				self.edges[disconnectedNodes[0]][connection] = cost
