...
def usage():
	print 'usage: python compileNesCCode.py --nesc-dir=[dir] --compile-target=[telosb_t2] --sensor-board=[micasb|mts300] --compile-targetNodes=[list]'
	sys.exit(2)

def parseArgs(args):
	global optNescDir, optCompileTarget, optSensorBoard, optTargetNodes

	try:
		optNames = ["help", "nesc-dir=", "compile-target=", "sensor-board=", "compile-targetNodes="]
...
def compileNesCCode(nescRootDir, target = "telosb_t2", sensorBoard = None):
	...
	if ((target == "telosb_t2") or (target == "avrora_mica2_t2") or (target == "avrora_micaz_t2")):
		for dir in os.listdir(nescRootDir):
			if (os.path.isdir(dir) and dir in optTargetNodes):
