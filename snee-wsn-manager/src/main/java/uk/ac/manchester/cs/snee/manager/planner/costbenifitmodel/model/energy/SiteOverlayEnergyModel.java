...
public class SiteOverlayEnergyModel extends SiteEnergyModel
{
   protected LogicalOverlayNetwork overlayNetwork;
   
   public SiteOverlayEnergyModel(AgendaIOT agenda, LogicalOverlayNetwork overlayNetwork)
   {
     super(agenda);
   ...
   * @throws SNEEConfigurationException 
   ...
   TypeMappingException , SNEEConfigurationException
   {
     ...
     for (int i=0; i<siteTasks.size(); i++) 
     {
       ...
       {
         if(!overlayNetwork.isClusterHead(t.getSiteID()))
         { 
           cpuActiveTimeBms -= t.getDuration();
         }
         else
       ...
       else if (t instanceof CommunicationTask)
       {
         ...
         if(!overlayNetwork.isClusterHead(ct.getSourceID()) && 
             ct.getMode()==CommunicationTask.TRANSMIT)
         { 
           cpuActiveTimeBms -= t.getDuration();
         }
         else
