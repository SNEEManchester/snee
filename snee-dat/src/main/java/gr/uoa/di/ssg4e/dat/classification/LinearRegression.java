...
class LinearRegression extends AbstractClassifier {
	...
	protected void validateParameters() throws DATException, ParserException {
		...
		/* Create a SNEEql query for the dat source. This should *NOT* throw
		 * an exception, as it SHOULD have been tested before */
	...
	private String getABComputationString(String inp_x, String inp_y){
		...
		return 
			"SELECT	( T.n * T.sum_xy - T.sum_y * T.sum_x ) / ( T.n * T.sum_sqr_x - T.sum_x * T.sum_x ) as a," +
			"		( T.sum_y * T.sum_sqr_x - T.sum_x * T.sum_xy ) / ( T.n * T.sum_sqr_x - T.sum_x * T.sum_x ) as b " +
			"FROM (" +
			"	SELECT	COUNT( S." + inp_x + ") as n," +
	...
	public String refactorQuery( SNEEqlQuery query, int datIndex ) throws DATException {
		...
		for ( int i = 0; i < args.length; i++ )
			 sb.append(' ').append(args[i].replaceAll("\\b" + derAtt.toLowerCase() + "\\b",
						datItr.toLowerCase() + ".a * " + bindings[0].toLowerCase().replaceAll("[() ]", "") + 
						" + " + datItr.toLowerCase() + ".b") ).append(",");
