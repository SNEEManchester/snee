...
public class SourceAllocator {
	 ...
	 * 
	 * If the operator is a Receive Operator, then the extent metadata
	 * contains the information about its arrival rate.
	 *
	 * If the operator is a Scan Operator, then it does not have an arrival rate.
	 * 
	...
	throws SourceMetadataException, SNEECompilerException {
		if (logger.isTraceEnabled()) {
			logger.trace("ENTER setSourceRates() with " + laf);
		}
		...
		while (opIter.hasNext()) {
			...
			if (op instanceof AcquireOperator) {
				...
				//XXX-AG No need to do anything. ReceiveOperator rate set from schema when operator created
//				ReceiveOperator receiveOperator = (ReceiveOperator) op;
//				SourceMetadataAbstract inputSource = receiveOperator.getSource();
//				double rate = receiveOperator.getExtentName()
//				op.setSourceRate(rate);
				...
				valveOperator.setSourceRate(op.getInput(0).getStreamRate());
			} else {
				op.setSourceRate(op.getInput(0).getStreamRate());
			}
		}
		if (logger.isTraceEnabled()) {
	...
	private Set<SourceMetadataAbstract> retrieveSources(LAF laf)
		...
		Iterator<LogicalOperator> opIter = laf.operatorIterator(TraversalOrder.PRE_ORDER);
		while (opIter.hasNext()) {
			LogicalOperator op = opIter.next();
			if (op instanceof InputOperator) {
				sources.add(((InputOperator) op).getSource());
			}
//			if (op instanceof AcquireOperator) {
//				AcquireOperator acquireOp = (AcquireOperator) op;
...
//			}
	...
	private void validateSources(Set<SourceMetadataAbstract> sources)
	throws SourceAllocatorException {
		if (logger.isTraceEnabled()) {
