...
import uk.ac.manchester.cs.snee.manager.planner.overlaysuccessorrelation.tabu.OverlayTabuSearch;
import uk.ac.manchester.cs.snee.metadata.MetadataManager;
import uk.ac.manchester.cs.snee.metadata.source.SourceMetadataAbstract;

public class OverlaySuccessorRelation extends AutonomicManagerComponent
{
  ...
  public OverlaySuccessorRelation(File plannerFolder, HashMap<String, RunTimeSite> runningSites,
                           MetadataManager _metadataManager, SourceMetadataAbstract _metadata, AutonomicManagerImpl manager)
  {
    this.manager = manager;
    this._metadata = _metadata;
    successorFolder = new File(plannerFolder.toString() + sep + "overlaySuccessorRelation");
  ...
  public OverlaySuccessorPath executeSuccessorRelation(SensorNetworkQueryPlan initialPoint)
  {
    try
    {
      OverlayTabuSearch search = null;
      //set up TABU folder
      ...
      //search though space
      search = new OverlayTabuSearch(manager, runningSites, _metadata, _metadataManager, TABUFolder);
      OverlaySuccessorPath bestSuccessorRelation = search.findSuccessorsPath(initialPoint);
      new PlannerUtils(successorFolder, this.manager).writeOverlaySuccessorToFile(bestSuccessorRelation.getSuccessorList(), "finalSolution");
   ...
   * @return
   */
  private OverlaySuccessorPath readInSuccessor()
  {
    try
    {
      //use buffering
      ...
      OverlaySuccessorPath recoveredsuccessor = (OverlaySuccessorPath)input.readObject();
      input.close();
  ...
  private void writeSuccessorPathToFile(OverlaySuccessorPath bestSuccessorRelation)
  {
    try
    {
